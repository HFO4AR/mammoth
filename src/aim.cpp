...
void PTZ::InitMotorDirection(bool is_positive_direction) {
    const int speed = is_positive_direction ? 500 : -500;
    
    yaw_motor_.SetSpdPid(1,0.2,0.0,10000);
    pitch_motor_.SetSpdPid(1,0.1,0.0,2000);
    bool pitch_init_complete = true;
    ...
    while(!pitch_init_complete || !yaw_init_complete) {
        ...
        if (!pitch_init_complete) {
            if (abs(pitch_motor_.GetPidOutput(kSpd)) > 1600) {
                ...
                pitch_motor_.SetSpdPid(8,0.1,0.0,5500);
...
void PTZ::Init()
{
    ...
    yaw_motor_.SetPosPid(0.5,0,4,500);
